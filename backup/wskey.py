...
def send_notification(title, content, summary):
    ...
    <section style="margin: 0px auto;text-align: left;border: 2px solid #212122;padding: 10px 0px;box-sizing:border-box; width: 100%; display:inline-block;" class="ipaiban-bc">
        <section style="margin-top: 1rem; float: left; margin-left: 1rem; font-size: 1.3rem; font-weight: bold;">
            <p style="margin: 0; color: black">texttext</p>
        </section>
        <section style="display: block;width: 0;height: 0;clear: both;"></section>
        <section style="margin-top:20px; display: inline-block; border-bottom: 1px solid #212122; padding: 4px 20px; box-sizing:border-box;" class="ipaiban-bbc">
            <section style="width:25px; height:25px; border-radius:50%; background-color:#212122;display:inline-block;line-height: 25px" class="ipaiban-bg">
                <p style="text-align:center;font-weight:1000;margin:0"><span style="color: #ffffff;font-size:20px;">📢</span></p>
            </section>
            <section style="display:inline-block;padding-left:10px;vertical-align: top;box-sizing:border-box;"></section>
        </section>
        <section style="margin-top:0rem;padding: 0.8rem;box-sizing:border-box;">
            <p style=" line-height: 1.6rem; font-size: 1.1rem; ">despdesp</p>            
        </section>
    </section>
</section>'''
...
def get_ep(jduuid: str=''):
    ...
    area = base64Encode('%s_%s_%s_%s' % (random.randint(1, 10000), random.randint(1, 10000), random.randint(1, 10000), random.randint(1, 10000)))
    d_model = random.choice(['Mi11Ultra', 'Mi11', 'Mi10'])
    d_model = base64Encode(d_model)
    return '{"hdid":"JM9F1ywUPwflvMIpYPok0tt5k9kW4ArJEU3lfLhxBqw=","ts":%s,"ridx":-1,"cipher":{"area":"%s","d_model":"%s","wifiBssid":"dW5hbw93bq==","osVersion":"CJS=","d_brand":"WQvrb21f","screen":"CtS1DIenCNqm","uuid":"%s","aid":"%s","openudid":"%s"},"ciphertype":5,"version":"1.2.0","appname":"com.jingdong.app.mall"}' % (int(ts) - random.randint(100, 1000), area, d_model, bsjduuid, bsjduuid, bsjduuid), jduuid, ts
...
def getcookie_wskey(key):
    ...
    for num in range(0, 5):
        ...
        try:
            res = get(url=url, params=params, verify=False, allow_redirects=False, proxies={"http": proxys, "https": proxys}).cookies.get_dict()        
        except Exception as error:
    ...
    if envtype in ('ql', ''):
        for data in datas:
            ...
            if "app_open" in cookie:
                ...
                if "fake_" in cookie:
                    ...
                    # --- 同时禁用 wskey 和面板内残留的对应 Cookie ---
                    disable_ids = []
                    
                    # 1. 加入当前 wskey 的 ID
                    try:
                        disable_ids.append(data['_id'])
                    except:   
                        disable_ids.append(data['id'])
                        
                    # 2. 动态检索面板，寻找并加入该 pin 的旧 JD_COOKIE ID
                    try:
                        ck_search_url = 'http://127.0.0.1:5600/api/envs'
                        ck_search_params = {'searchValue': pin}
                        ck_datas = get(ck_search_url, params=ck_search_params, headers=headers).json().get('data', [])
                        
                        for ck_data in ck_datas:
                            if "JD_COOKIE" in ck_data.get('name', '') and pin in ck_data.get('value', ''):
                                try:
                                    disable_ids.append(ck_data['_id'])
                                except:
                                    disable_ids.append(ck_data['id'])
                                printf(f"成功找到账号 {newpin} 对应的旧 Cookie，加入禁用列表")
                    except Exception as ck_err:
                        printf(f"尝试搜索旧 Cookie 时出错: {ck_err}")

                    # 3. 批量禁用到青龙面板
                    if disable_ids:
                        url = 'http://127.0.0.1:5600/api/envs/disable'
                        put(url, json=disable_ids, headers=headers)                
                        printf(f"禁用成功: {newpin} (已同时下线 wskey 及对应 Cookie)")
                        resurt2 = resurt2 + f"wskey及Cookie已禁用:{newpin}\n"
                else:
        ...
        for ws, remark in wslist:
            ...
            if "app_open" in cookie:
                ...
                if "fake_" in cookie:
                    message = f"{remark}@{pin}，wskey可能过期了！ ❌\n"
                    printf(message)
                    resurt2 = resurt2 + f"{remark}@{pin}，wskey可能过期了！ ❌\n"
                else:
